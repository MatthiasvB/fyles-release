...
use fyles_core::core::{
        brain::{
            ...
            types::ContactShareChallenge,
        },
        domain_models::Contact,
    };
...
impl<T: FileTracker + 'static, S: LocalNetworkSwarm + 'static> RefCountEventLoopData<T, S> {
    #[instrument(skip_all)]
    pub fn handle_contact_share_event(
        self,
        event: Event<
        ...
        tokio::spawn(async move {
            match event {
                Event::Message {
                    ...
                    match message {
                        request_response::Message::Request {
                            ...
                            debug!("Received contact invite request from {peer}");
                            tokio::spawn(async move {
                                if let Some(response) = self.decrypt_respond(peer, request, request_id).await {
                                    self.with_swarm(move |swarm| {
                                        if swarm.get_contact_share_behaviour().send_response(
                                            channel,
                                            response.map_err(|_e| SessionErrorResponse::CryptError),
                                        ).is_err() {
                                            warn!(?peer, "Unable to send response to peer");
                                        };
                                    })
                                } else {
                                    warn!("Invalid contact invite challenge from {peer}");
                                    let res = self.brain_action_sender.send(BrainAction::NetworkNode(NetworkNodeAction::RejectedContactShare)).await;
                                    if res.is_err() {
                                        error!("Send error");
                                    }
                        ...
                        request_response::Message::Response {
                            ...
                            self.receive_encrypted_response(peer, request_id, payload, &self.contact_invite_challenge_request_registry).await;
                        }
                    }
                }
                Event::OutboundFailure {
                    ...
                    self.brain_action_sender.send(BrainAction::NetworkNode(NetworkNodeAction::ContactShareGotRejected)).await.expect("Sending to work");
                }
